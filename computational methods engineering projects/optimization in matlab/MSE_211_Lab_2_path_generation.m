...
for i = 1:1:3
   ...
   gx(i) = r4x + r3bx + inputs.deltax(i) - r3bx*cos(gamma(i)) + r3by*sin(gamma(i)) ...
            -r4x*cos(alpha(i)) + r4y*sin(alpha(i));
        gy(i) = r4y + r3by + inputs.deltay(i) - r3by*cos(gamma(i)) - r3bx*sin(gamma(i)) ...
            -r4y*cos(alpha(i)) - r4x*sin(alpha(i));
end
...
for i=1:length(theta_2)
    ...
    % Check if the square root argument is negative
if (a^2 + b^2 - c^2) < 0
    disp('Square root of a negative number encountered. Mechanism configuration might be incorrect.');
