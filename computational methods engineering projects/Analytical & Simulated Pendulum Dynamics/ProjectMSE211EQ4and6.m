...
for i = 1:length(theta0_values)
    theta0 = theta0_values(i); figure(i)
    for lambda = lambda_values
    f = @(t, theta, omega) -(lambda*omega + (m*g*sin(theta))/r)/m;
    [t, theta, omega] = rungeKutta4SecondOrder(f, tspan, theta0*pi/180, 0, h);
    plot(t, omega*180/pi, 'DisplayName', ['\lambda = ' num2str(lambda)]);
    hold on 
    end

    xlabel('Time (s)')
    ylabel('Angular velocity (deg/s)')
    title(['Equation 4: Angular velocity vs time for \theta_0 = ',num2str(theta0) '\circ']) 
...
for i = 1:length(theta0_values) 
    ...
    xlabel('Time (s)')
    ylabel('Angular velocity (deg/s)')
    title(['Equation 6: Angular velocity vs time for \theta_0 = ',num2str(theta0) '\circ']) 
    legend('Location', 'northwest') 
    hold off
end
